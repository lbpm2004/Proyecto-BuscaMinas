...
public class Tablero {
    ...
    private boolean[][] minaLugar; //matriz para saber si hay una mina en las casillas
...
public Tablero(Casilla[][] tablero, int filas, int columnas, int minas){
    this.filas = filas;
    this.columnas = columnas;
    this.minas = minas;
    this.minaLugar = new boolean[filas][columnas]; //Inicializa la matriz
...
private void ponerMinas() {
    ...
    while (minasPuestas < minas) { //mientras que minasPuestas sea menor que las minas en el tablero
        ...
        if (!minaLugar[fila][columna]) { //Ver si no hay minas en esa posicion
            minaLugar[fila][columna] = true; //Pone la mina
...
private void calcularCantidadMinas() {
    for (int i = 0; i < filas; i++) { //Recorre cada fila  //i es el indice de la fila actual
        for (int j = 0; j < columnas; j++) { //Recorre cada columna  //j es el indice de la columna actual
            if (minaLugar[i][j]) { //Si hay una mina en la posicion en la que estamos
...
public boolean descubrirCasilla(int fila, int columna) { //devuelve un boolean y le pasamos fila y columna
    if (minaLugar[fila][columna]) { //ver si hay una mina en la casilla indicada por fila y columna 
...
public boolean isCasillaConMina(int fila, int columna) {
    return minaLugar[fila][columna]; //Si hay o no una mina en la casilla
}
